...
BEGIN
SET @output +=
'SET IDENTITY_INSERT ' + @Target_Table_For_Output + ' ON
...
BEGIN
SET @output +=
'ALTER TABLE ' + @Target_Table_For_Output + ' NOCHECK CONSTRAINT ALL
...
SET @output +=
'MERGE INTO ' + @Target_Table_For_Output + ' AS Target
USING (VALUES
'
...
BEGIN
SET @output += CAST((SELECT val + @b FROM @tab FOR XML PATH('')) AS XML).value('.', 'VARCHAR(MAX)');
END
...
BEGIN
SET @output +=
'WHEN MATCHED ' + CASE WHEN @update_only_if_changed = 1 THEN 'AND (' + @Column_List_For_Check + ') ' ELSE '' END + 'THEN
 UPDATE SET
  ' + LTRIM(@Column_List_For_Update) + '
'
END
...
'WHEN NOT MATCHED BY TARGET THEN
 INSERT(' + @Column_List + ')
 VALUES(' + REPLACE(@Column_List, '[', 'Source.[') + ')
'
...
IF @mergeError != 0
BEGIN
	PRINT ''ERROR OCCURRED IN MERGE FOR ' + @Target_Table_For_Output + '. Rows affected: '' + CAST(@mergeCount AS VARCHAR(100)); -- SQL should always return zero rows affected
END
ELSE
BEGIN
	PRINT ''' + @Target_Table_For_Output + ' rows affected by MERGE: '' + CAST(@mergeCount AS VARCHAR(100));
END
...
BEGIN
SET @output +=
'ALTER TABLE ' + @Target_Table_For_Output + ' CHECK CONSTRAINT ALL
...
BEGIN
SET @output +=
'SET IDENTITY_INSERT ' + @Target_Table_For_Output + ' OFF
